...
```bash
cp $WORK_DIR/investigation_report.md ~/.claude/ultragent/generations/$NEXT_GEN/ 2>/dev/null
```
...
```bash
PYTHONIOENCODING=utf-8 python ~/.claude/ultragent/circuit_breaker.py check opus
```
...
```bash
PYTHONIOENCODING=utf-8 python ~/.claude/ultragent/circuit_breaker.py record opus success
...
```bash
PYTHONIOENCODING=utf-8 python ~/.claude/ultragent/context_engine.py assemble --model opus
```
